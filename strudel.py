...
class Strudel:
	...
	**** Normal random, no cluster structure, sine spike
	...
	def imbalanced_synthetic_dataset_uniform(self, D, N, B, cluster_percentage = 1, within_noise = 0.5, between_noise = 0.5, association_type = 'parabola' ):
		"""
			D: int
				number of features
		...
		X_base = self.orthogonalize_matrix(X_base)
		Y_base = self.orthogonalize_matrix(Y_base)
		#X = self.orthogonalize_matrix(X)
		#Y = self.orthogonalize_matrix(Y)
		...
		if association_type == "L":
			...
			common_base_Y = self.orthogonalize_matrix(common_base_Y)
		#common_base = self.orthogonalize_matrix(common_base)
		...
		for i in  range(int(D*cluster_percentage)):
			...
			if association_type == "parabola":
				...
				Y[i] = [ self.happyface(common_base[l,k]) + within_noise * numpy.random.uniform(low=-.1,high=.1, size=1) for k in range(N)]
